...
class PGAgent(BaseAgent):
    ...
    def calculate_baselines(self, paths):
        ...
        for path in paths:
            ...
            baseline_predictions = self.actor.run_baseline_prediction(observations)
            path["baseline"] = baseline_predictions[:-1]
    ...
    def calculate_returns(self, paths):
        ...
        for path in paths:
            ...
            path["return"] = self._discounted_cumsum(rewards + [self.gamma * baselines[-1]])
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def calculate_advantages(self, paths):
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        for path in paths:
            path["advantage"] = path["return"]

        if self.standardize_advantages:        
            path["advantage"] = path["return"] - path["baseline"]

        if self.standardize_advantages:
            all_advantages = np.concatenate([path["advantage"] for path in paths])
            mean_advantages = np.mean(all_advantages)
            std_advantages = np.std(all_advantages) + 1e-8  # add a small value to avoid division by zero
            for path in paths:
                path["advantage"] = (path["advantage"] - mean_advantages) / std_advantages
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in range(T):
            cumsum = np.sum(rewards[t:] * discounts[:T-t])
            list_of_discounted_cumsums.append(cumsum)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
